...
rule dh_reuse_alignreject:
    ...
    output:
        reject_1=temp(config["inputOutput"]["output_dir"]+"/{sample}/align_reject/reject_R1.fastq.gz"),
        reject_2=temp(config["inputOutput"]["output_dir"]+"/{sample}/align_reject/reject_R2.fastq.gz"),
    log:
...
rule dh_hostalign:
    ...
    output:
        host_aln=temp(config["inputOutput"]["output_dir"]+"/{sample}/dh_hostalign/host_aln.sam"),
    log:
    ...
    conda:
        config["tools"]["dehuman"]["conda"]
    benchmark:
        config["inputOutput"]["output_dir"]+"/logs/benchmark/dh_hostalign/{sample}/host_aln.benchmark"
    resources:
        disk_mb=1250,
...
rule dh_filter:
    ...
    output:
        filter_count=config["inputOutput"]["output_dir"]+"/{sample}/dh_filter/{sample}_dehuman.count",
        filter_list=temp(config["inputOutput"]["output_dir"]+"/{sample}/dh_filter/{sample}_dehuman.filter"),
        filtered_1=temp(config["inputOutput"]["output_dir"]+"/{sample}/dh_filter/{sample}_dehuman_R1.fastq.gz"),
        filtered_2=temp(config["inputOutput"]["output_dir"]+"/{sample}/dh_filter/{sample}_dehuman_R2.fastq.gz"),
    params:
...
rule dehuman:
    ...
    output:
        ...
        final_cram=config["inputOutput"]["output_dir"]+"/{sample}/dehuman/{sample}_dehuman.cram",
        checksum=config["inputOutput"]["output_dir"]+"/{sample}/dehuman/{sample}_dehuman.cram.%s" % config["tools"]["dehuman"]["checksum"],
    params:
